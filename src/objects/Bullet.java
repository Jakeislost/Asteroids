...
public class Bullet extends GameObject{
	...
	private int life, // life frame counter
				lifeFrames = 30, UFOLifeModifier = 4; // total frames before death
	private boolean hit; // in order to stop concurrent modification error
	
	private boolean friendly;
	
	public Bullet(float x, float y, float angle, boolean friendly) {
		super(x, y);
		this.angle = angle;
	...
	public void update(GameContainer gc, StateBasedGame gsm, int delta, ArrayList<GameObject> object) {
		if(!hit) {
			life ++; // increase life frames
	        if(life > ((!friendly) ? lifeFrames * UFOLifeModifier : lifeFrames)) object.remove(this);
	        //move bullet;
