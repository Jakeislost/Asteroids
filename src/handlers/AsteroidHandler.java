...
public class AsteroidHandler {
	...
	private FrameTimer ufoTimer, asteroidTimer;
	...
	public AsteroidHandler(Handler handler) {
		...
		asteroidTimer = new FrameTimer(500, false);
		ufoTimer = new FrameTimer(rand.nextInt(500) + 2000, false);
	}	
	
	public void update() {
		asteroidTimer.update();
		if(asteroidTimer.isDone()) {
			for(int i = 0; i < 2; i++) {
				handler.addOject(new Asteroid(rand.nextInt(Game.GAME_END_X - Game.GAME_START_X), 25, AsteroidType.large, (float) Math.toRadians(rand.nextInt(360))));
			}
			asteroidTimer.resetTimer();
		}
